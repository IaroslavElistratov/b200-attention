// Kernel 17 focus: keep barrier parity cursors in register bitmasks.
//   Indexed phase arrays can become local-memory traffic when their values
//   persist across work items. One bit per barrier stage carries the same state
//   without changing waits, arrivals, barrier lifetimes, or kernel math.
...
__global__ __launch_bounds__(TB_SIZE, 1) void attention_tcgen05_v17_kernel(
...
void attention_tcgen05_v17_launch(const nv_bfloat16* Q_ptr, const nv_bfloat16* K_ptr,
  ...
  if (smem_bytes > 48 * 1024) {
    check_cuda(cudaFuncSetAttribute(attention_tcgen05_v17_kernel,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    int(smem_bytes)));
  }

  attention_tcgen05_v17_kernel<<<grid, TB_SIZE, smem_bytes, stream>>>(
