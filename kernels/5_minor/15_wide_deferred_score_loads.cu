// Kernel 15 focus: overlap score acquisition with rowmax work.
//   1. Load S from TMEM with eight x16 nowait operations instead of sixteen x8 waits.
//   2. Reduce each x16 fragment through four independent rowmax chains.
//   3. Defer the one required TMEM-load wait until immediately before P overwrites S.
...
__global__ __launch_bounds__(TB_SIZE, 1) void attention_tcgen05_v15_kernel(
  ...
  if (is_mma_warp && elect_sync()) {
    // Barriers are initialized once, so these phase arrays persist across work items.
  ...
  if (is_softmax_warp) {
    ...
    for (int work_id = blockIdx.x, work_iter = 0; work_id < total_work_items;
      ...
      for (int kv_tile = 0; kv_tile < kv_tiles; ++kv_tile) {
        ...
        // Acquire S with x16 nowait loads and reduce through four independent
        // rowmax chains; the wait::ld is deferred into the P pass.
...
#pragma unroll
        ...
        // Keep Kernel 14's scalar rowsum while changing only score acquisition.
...
void attention_tcgen05_v15_launch(const nv_bfloat16* Q_ptr, const nv_bfloat16* K_ptr,
  ...
  if (smem_bytes > 48 * 1024) {
    check_cuda(cudaFuncSetAttribute(attention_tcgen05_v15_kernel,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    int(smem_bytes)));
  }

  attention_tcgen05_v15_kernel<<<grid, TB_SIZE, smem_bytes, stream>>>(
