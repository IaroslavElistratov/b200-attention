// Direct contiguous-BSHD I/O variant of Kernel 18.
//
// Global BSHD is viewed by TMA as a 2D [B * L, H * D] matrix. Each work item
// selects one head with a head_id * HEAD_DIM x-offset. SMEM layouts and the
// complete attention schedule remain unchanged.
//
// Kernel 18 focus: request 256-byte L2 promotion for Q/K/V TMA tensor maps.
//   Each TMA box row consumes one 128-byte half of an aligned 256-byte sector.
//   Promoting the full sector can leave the adjacent box's half resident in L2.
//   The kernel body, tensor-map coordinates, and transferred values are unchanged.
...
// The tensor map views global memory as [B * L, H * D]; global_col0 selects the
// head while the existing SW128 SMEM layout remains unchanged.
__device__ __forceinline__ void tma_load_kmajor(
  ...
  for (int k = 0; k < HEAD_DIM / SW128_ATOM_COLS; ++k) {
    ...
    tma::copy_2d_gmem_to_smem(dst, tmap, global_col0 + off_k, global_row0,
                              mbar_addr);
  }
}

// Load one head's V tile into the same canonical MN-major SW128 SMEM layout.
__device__ __forceinline__ void tma_load_mnmajor(
    int dst_smem_base, const CUtensorMap* tmap, int global_col0, int global_row0,
    int mbar_addr) {
  const int n64_stride = SW128_ATOM_ROWS * SW128_BYTES;
  const int k8_stride = (HEAD_DIM / SW128_ATOM_COLS) * n64_stride;
  ...
  for (int k8 = 0; k8 < BLOCK_N / SW128_ATOM_ROWS; ++k8) {
    ...
    for (int n64 = 0; n64 < HEAD_DIM / SW128_ATOM_COLS; ++n64) {
      ...
      const int dst = dst_smem_base + k8 * k8_stride + n64 * n64_stride;
...
__device__ __forceinline__ void load_token_and_arrive(
    int token, int batch_id, int head_col0, int len_kv, int KV_smem,
    int mbar_kv_ready_base, const CUtensorMap* K_tmap,
  ...
  if (is_k_token(token)) {
    bshd_io::tma_load_kmajor(smem_addr, K_tmap, BLOCK_N, head_col0, kv_row0,
                            mbar_kv_ready_addr);
  } else {
    bshd_io::tma_load_mnmajor(smem_addr, V_tmap, head_col0, kv_row0,
                             mbar_kv_ready_addr);
  }
...
__global__ __launch_bounds__(TB_SIZE, 1) void attention_tcgen05_v18_bshd_kernel(
    ...
    int len_kv, int heads, int total_work_items) {
  ...
  if (is_load_warp && elect_sync()) {
    // Publish the layout parameter during CTA setup so it is not live in the
    // register-heavy softmax loop. The following CTA sync makes it visible.
    layout_heads[0] = heads;
  ...
  //
  // work_id identifies one batch/head and one Q0/Q1 output-tile pair.
  ...
  if (is_load_warp && elect_sync()) {
    ...
    for (int work_id = blockIdx.x, work_iter = 0; work_id < total_work_items;
      ...
      // 3) Load Q once
      //
      // Decode this work item into one batch/head and one Q0/Q1 pair. Contiguous
      // BSHD is viewed as [B * L, H * D].
      const int work_items_per_head = len_q / q_stage::ROWS_PER_WORK_ITEM;
      const int batch_head_id = work_id / work_items_per_head;
      const int q_pair_id = work_id - batch_head_id * work_items_per_head;
      const int work_heads = layout_heads[0];
      const int batch_id = batch_head_id / work_heads;
      const int head_id = batch_head_id - batch_id * work_heads;
      const int q_row0_base =
          batch_id * len_q + q_pair_id * q_stage::ROWS_PER_WORK_ITEM;
      ...
      for (int q = 0; q < q_stage::STAGES; ++q) {
        ...
        bshd_io::tma_load_kmajor(Q_stage_smem, &Q_tmap, BLOCK_M, head_col0,
                                 q_row0_stage, mbar_q_ready_addr);
      }
      ...
      for (int token = 0; token < prefill_tokens; ++token) {
        kv_ring::load_token_and_arrive(
            token, batch_id, head_col0, len_kv, KV_smem, mbar_kv_ready_base,
            &K_tmap, &V_tmap);
      }
  ...
  if (is_mma_warp && elect_sync()) {
    // One parity bit per barrier stage; the masks persist across work items.
    unsigned phase_kv_ready_bits = 0;
    unsigned phase_p_high96_ready_o_safe_bits = 0;
    unsigned phase_p_full_ready_bits = 0;
    for (int work_id = blockIdx.x, work_iter = 0; work_id < total_work_items;
         work_id += gridDim.x, ++work_iter) {
      ...
      mbarrier::wait(mbar_kv_ready_addr0, ((phase_kv_ready_bits >> (k_stage0)) & 1u));
      phase_kv_ready_bits ^= 1u << (k_stage0);
      asm volatile("tcgen05.fence::after_thread_sync;\n" ::: "memory");
      ...
      for (int kv_tile = 0; kv_tile < kv_tiles; ++kv_tile) {
        ...
        mbarrier::wait(mbar_v_ready_addr, ((phase_kv_ready_bits >> (v_stage)) & 1u));
        phase_kv_ready_bits ^= 1u << (v_stage);
        asm volatile("tcgen05.fence::after_thread_sync;\n" ::: "memory");
        ...
        for (int q = 0; q < q_stage::STAGES; ++q) {
          ...
          mbarrier::wait(mbar_p_high96_ready_o_safe_addr,
                         ((phase_p_high96_ready_o_safe_bits >> (q)) & 1u));
          phase_p_high96_ready_o_safe_bits ^= 1u << (q);
          asm volatile("tcgen05.fence::after_thread_sync;\n" ::: "memory");
          ...
          mbarrier::wait(mbar_p_full_ready_addr, ((phase_p_full_ready_bits >> (q)) & 1u));
          phase_p_full_ready_bits ^= 1u << (q);
          asm volatile("tcgen05.fence::after_thread_sync;\n" ::: "memory");
          ...
          if (have_next) {
            ...
            if (q == 0) {
              mbarrier::wait(next_mbar_kv_ready_addr, ((phase_kv_ready_bits >> (next_k_stage)) & 1u));
              phase_kv_ready_bits ^= 1u << (next_k_stage);
              asm volatile("tcgen05.fence::after_thread_sync;\n" ::: "memory");
            }
...
#pragma unroll
        ...
        nv_bfloat16* out_ptr =
            O_ptr + (q_row0_stage + row) * (work_heads * HEAD_DIM) + head_col0 + out_n8 * 8;
  ...
  if (is_corr_warp) {
    ...
    // One parity bit per q-stage; the masks persist across work items.
    unsigned phase_stats_ready_bits = 0;
    unsigned phase_pv_done_bits = 0;
    const int row_base = local_warp_idx * WARP_SIZE;  // first of 32 rows owned by this warp
    const int row = row_base + lane_id;               // specific row owned by this lane
    const int trow = row_base << 16;                  // encode row_base for TMEM addressing
    ...
    for (int work_id = blockIdx.x, work_iter = 0; work_id < total_work_items;
      ...
      for (int kv_tile = 0; kv_tile < kv_tiles; ++kv_tile) {
        ...
        for (int q = 0; q < q_stage::STAGES; ++q) {
          ...
          mbarrier::wait(mbar_stats_ready_addr, ((phase_stats_ready_bits >> (q)) & 1u));
          phase_stats_ready_bits ^= 1u << (q);
          if (!first_kv) {
            ...
            mbarrier::wait(mbar_pv_done_addr, ((phase_pv_done_bits >> (q)) & 1u));
            phase_pv_done_bits ^= 1u << (q);
          }
...
// Local copy of init_tmap_2d_simple with 256-byte L2 promotion enabled.
inline void init_tmap_2d_l2prom256(CUtensorMap* tmap, const nv_bfloat16* ptr,
  ...
  auto err = cuTensorMapEncodeTiled(
      tmap, CUtensorMapDataType::CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, rank, (void*)ptr,
...
void attention_tcgen05_v18_bshd_launch(
  ...
  init_tmap_2d_l2prom256(&Q_tmap, const_cast<nv_bfloat16*>(Q_ptr), /*global_height=*/Q_height,
                      /*global_width=*/global_width,
                      /*shared_height=*/BLOCK_M, /*shared_width=*/SW128_ATOM_COLS,
                      CU_TENSOR_MAP_SWIZZLE_128B);

  init_tmap_2d_l2prom256(&K_tmap, const_cast<nv_bfloat16*>(K_ptr), /*global_height=*/KV_height,
                      /*global_width=*/global_width,
                      /*shared_height=*/BLOCK_N, /*shared_width=*/SW128_ATOM_COLS,
                      CU_TENSOR_MAP_SWIZZLE_128B);

  init_tmap_2d_l2prom256(&V_tmap, const_cast<nv_bfloat16*>(V_ptr), /*global_height=*/KV_height,
                      /*global_width=*/global_width,
                      /*shared_height=*/SW128_ATOM_ROWS, /*shared_width=*/SW128_ATOM_COLS,
                      CU_TENSOR_MAP_SWIZZLE_128B);

  const int total_work_items = batch * heads * (len_q / q_stage::ROWS_PER_WORK_ITEM);
  ...
  if (smem_bytes > 48 * 1024) {
    check_cuda(cudaFuncSetAttribute(attention_tcgen05_v18_bshd_kernel,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    int(smem_bytes)));
  }

  attention_tcgen05_v18_bshd_kernel<<<grid, TB_SIZE, smem_bytes, stream>>>(
      Q_tmap, K_tmap, V_tmap, O_ptr, len_q, len_kv, heads, total_work_items);
