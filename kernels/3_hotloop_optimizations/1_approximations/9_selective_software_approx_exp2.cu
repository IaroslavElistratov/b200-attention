// Kernel 9 focus: selective software approximate exp2 without split correction.
//   1. Keep kernel 8's combined row-warp topology and log2/exp2 path.
//   2. Add selective software E2E exp2 for a fraction of P exponentials.
//   3. Keep the full TMEM reread path for score fragments.
...
__global__ __launch_bounds__(TB_SIZE) void attention_tcgen05_v9_kernel(
...
#pragma unroll
            for (int j = 0; j < 4; ++j) {
              // Convert raw scores to exp2-input units.
              const float score0 = s8[2 * j] * softmax_scale_log2;
              const float score1 = s8[2 * j + 1] * softmax_scale_log2;
              // Now scores and rowmax are in the same log2 basis, so we can subtract directly.
              ...
              const int pair_start_in_block = load8 * 8 + 2 * j;
...
void attention_tcgen05_v9_launch(const nv_bfloat16* Q_ptr, const nv_bfloat16* K_ptr,
  ...
  if (smem_bytes > 48 * 1024) {
    check_cuda(cudaFuncSetAttribute(attention_tcgen05_v9_kernel,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    int(smem_bytes)));
  }

  attention_tcgen05_v9_kernel<<<grid, TB_SIZE, smem_bytes, stream>>>(
