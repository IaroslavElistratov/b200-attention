// Kernel 8 focus: hardware approximate exp2 without split correction.
//   1. Keep kernel 7's combined row-warp topology and q-stage compute braid.
//   2. Move row softmax into the log2 domain and use hardware approx exp2.
//      Compute rowmax on raw scores, then scale the final max once into log2 space.
//   3. Keep O rescale inline in the row warps.
...
__global__ __launch_bounds__(TB_SIZE) void attention_tcgen05_v8_kernel(
...
#pragma unroll
            for (int j = 0; j < 4; ++j) {
              ...
              const float p0 = approx::fast_exp2(score0 - rowmax);
              const float p1 = approx::fast_exp2(score1 - rowmax);
              tile_rowsum += p0 + p1;
...
void attention_tcgen05_v8_launch(const nv_bfloat16* Q_ptr, const nv_bfloat16* K_ptr,
  ...
  if (smem_bytes > 48 * 1024) {
    check_cuda(cudaFuncSetAttribute(attention_tcgen05_v8_kernel,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    int(smem_bytes)));
  }

  attention_tcgen05_v8_kernel<<<grid, TB_SIZE, smem_bytes, stream>>>(
