// Kernel 10 focus: avoid most score TMEM rereads without split correction.
//   1. Keep kernel 9's combined row-warp topology and selective E2E path.
//   2. Cache the high96 score values during rowmax and reread only the low32 tail.
//   3. Keep O correction inline in the row warps.
// Retain high96 scores during rowmax, consume them in descending 16-column chunks to shorten
// their lifetimes, reread low32, and relocate packed P so those unread scores remain intact.
...
__global__ __launch_bounds__(TB_SIZE) void attention_tcgen05_v10_kernel(
...
#pragma unroll
      ...
      const float rescale = approx::fast_exp2(rowmax - new_rowmax);
      rowmax = new_rowmax;
      rowsum *= rescale;
...
#pragma unroll
            for (int j = 0; j < 4; ++j) {
              ...
              // rowmax is already in exp2-input units; scale the raw scores to match
              // const float score0 = s8[2 * j] * softmax_scale_log2;
              // const float score1 = s8[2 * j + 1] * softmax_scale_log2;
              // subtract the row maximum for stable softmax
              // const float shifted_score0 = score0 - rowmax;
              // const float shifted_score1 = score1 - rowmax;
...
void attention_tcgen05_v10_launch(const nv_bfloat16* Q_ptr, const nv_bfloat16* K_ptr,
  ...
  if (smem_bytes > 48 * 1024) {
    check_cuda(cudaFuncSetAttribute(attention_tcgen05_v10_kernel,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    int(smem_bytes)));
  }

  attention_tcgen05_v10_kernel<<<grid, TB_SIZE, smem_bytes, stream>>>(
