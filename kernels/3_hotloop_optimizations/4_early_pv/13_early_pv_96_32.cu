// Kernel 13 focus: shift from producing P faster to letting the MMA warp consume it earlier
// Recent kernels shortened the row-side softmax/P path, MMA's main exposed dependency
//   1. Keep kernel 12's split correction + cache96/reread32 + skip-rescale path
//   2. Publish high96 P + O-safe so MMA can start PV before low32 P is ready
...
__global__ __launch_bounds__(TB_SIZE) void attention_tcgen05_v13_kernel(
  ...
  const int mbar_p_high96_ready_o_safe_base =
      static_cast<int>(__cvta_generic_to_shared(mbar_p_high96_ready_o_safe));
  ...
  if (is_load_warp && elect_sync()) {
    ...
    for (int q = 0; q < q_stage::STAGES; ++q) {
      ...
      mbarrier::init(mbar_p_high96_ready_o_safe_addr,
  ...
  if (is_softmax_warp) {
    ...
    const int mbar_p_high96_ready_o_safe_addr =
        q_stage::mbar_addr(mbar_p_high96_ready_o_safe_base, softmax_stage);
...
void attention_tcgen05_v13_launch(const nv_bfloat16* Q_ptr, const nv_bfloat16* K_ptr,
  ...
  if (smem_bytes > 48 * 1024) {
    check_cuda(cudaFuncSetAttribute(attention_tcgen05_v13_kernel,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    int(smem_bytes)));
  }

  attention_tcgen05_v13_kernel<<<grid, TB_SIZE, smem_bytes, stream>>>(
