// Kernel 12 focus: split O rescale from softmax so it can overlap P production.
//   1. Keep kernel 11's cache96/reread32 + E2E + skip-rescale row policy.
//   2. Softmax publishes per-row rescale through stats_ready, then continues the bottleneck
//      exp2/pack/store P path while correction warps rescale old O in parallel.
//   3. Joined p_ready_o_safe completes only after full P is ready and old O is safe;
//      MMA still waits once, then issues the whole PV.
// Final normalization and global output remain in the softmax warps
...
//   5) MMA warp waits for full Pq + Oq-safe, then runs the whole PV
...
__global__ __launch_bounds__(TB_SIZE) void attention_tcgen05_v12_kernel(
  ...
  //   p-ready-o-safe[q]:  softmax+correction warps -> mma warp after full Pq and O-safe
  ...
  // Kernel 11 called this softmax_done because its row warps produced both P and rescaled O
  // Here softmax contributes P-ready and correction contributes O-safe, so the joined barrier
  // is renamed p_ready_o_safe
  __shared__ uint64_t mbar_p_ready_o_safe[q_stage::STAGES];
  ...
  const int mbar_p_ready_o_safe_base =
      static_cast<int>(__cvta_generic_to_shared(mbar_p_ready_o_safe));
  ...
  if (is_load_warp && elect_sync()) {
    ...
    for (int q = 0; q < q_stage::STAGES; ++q) {
      ...
      const int mbar_p_ready_o_safe_addr = q_stage::mbar_addr(mbar_p_ready_o_safe_base, q);
      ...
      mbarrier::init(mbar_p_ready_o_safe_addr,
  ...
  if (is_mma_warp) {
    ...
    //   Pq storage starts at +64 b32 columns for the cache96/reread32 layout
  ...
  if (is_mma_warp && elect_sync()) {
    ...
    int phase_p_ready_o_safe[q_stage::STAGES] = {0, 0};
    ...
    for (int kv_tile = 0; kv_tile < kv_tiles; ++kv_tile) {
      ...
      for (int q = 0; q < q_stage::STAGES; ++q) {
        ...
        mbarrier::wait(mbar_p_ready_o_safe_addr, phase_p_ready_o_safe[q]);
        phase_p_ready_o_safe[q] ^= 1;
        asm volatile("tcgen05.fence::after_thread_sync;\n" ::: "memory");
  ...
  if (is_softmax_warp) {
    ...
    // local phase cursor for that qk_done stream.
    int phase_qk_done = 0;
    ...
    const int mbar_p_ready_o_safe_addr =
        q_stage::mbar_addr(mbar_p_ready_o_safe_base, softmax_stage);
...
#pragma unroll
      ...
      float tile_rowsum = 0.0f;
      {
        ...
        // consume cached high96 in descending 16-column chunks to shorten register lifetimes
        // then reread low32 from TMEM before publishing full P
#pragma unroll
...
#pragma unroll
      ...
      __syncwarp();
      if (lane_id == 0) {
        mbarrier::arrive(mbar_p_ready_o_safe_addr);
      }
...
void attention_tcgen05_v12_launch(const nv_bfloat16* Q_ptr, const nv_bfloat16* K_ptr,
  ...
  if (smem_bytes > 48 * 1024) {
    check_cuda(cudaFuncSetAttribute(attention_tcgen05_v12_kernel,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    int(smem_bytes)));
  }

  attention_tcgen05_v12_kernel<<<grid, TB_SIZE, smem_bytes, stream>>>(
