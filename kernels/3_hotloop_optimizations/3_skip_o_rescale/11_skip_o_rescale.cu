// Kernel 11 focus: defer rowmax rebasing to skip O rescale without split correction.
//   1. Keep kernel 10's cache96/reread32 + E2E row path.
//   2. If rowmax does not rise too much (less than 8 log2 units), keep the old softmax
//      reference and use rescale 1; if all 32 rows do this, skip the O pass.
//   3. Keep softmax/P publication and O correction in the same row warps.
...
// define which S fragments stay in registers and which are reread from TMEM
// one tcgen05 x8 TMEM load returns 8 S scores per row-warp thread
// 128 scores form 16 x8 fragments: cache the high 12 and reread the low 4
...
__global__ __launch_bounds__(TB_SIZE) void attention_tcgen05_v11_kernel(
  ...
  // Inherited from kernel 7: PVq(i) -> QKq(i+1) immediately reuses
  // that q-stage's freed S/P slot.
...
#pragma unroll
      ...
      if (!first_kv) {
        // the O load/store is warp collective, so all 32 lanes must execute it together
        // therefore run the O pass if even one lane's row needs rebasing
        // FULL_WARP_MASK is a 32-bit mask selecting all threads in this warp
        // each selected lane supplies its own predicate; any_sync returns true to every selected
        // lane if at least one selected lane supplies true
        // lanes whose rows do not rebase still participate and multiply O by 1
        const bool any_row_needs_o_rescale =
            __any_sync(FULL_WARP_MASK, row_needs_o_rescale);
...
void attention_tcgen05_v11_launch(const nv_bfloat16* Q_ptr, const nv_bfloat16* K_ptr,
  ...
  if (smem_bytes > 48 * 1024) {
    check_cuda(cudaFuncSetAttribute(attention_tcgen05_v11_kernel,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    int(smem_bytes)));
  }

  attention_tcgen05_v11_kernel<<<grid, TB_SIZE, smem_bytes, stream>>>(
