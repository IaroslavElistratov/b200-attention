// Kernel 14 focus: add persistent CTA scheduling without an all-warp work-item boundary.
//   1. Launch up to one CTA per SM and let each CTA process multiple output work items.
//   2. Initialize work-local barriers once per CTA and carry their phases forward.
//   3. Use q_free and KV recycle edges so each warp role advances when its data is safe.
...
__global__ __launch_bounds__(TB_SIZE, 1) void attention_tcgen05_v14_kernel(
  ...
  if (is_mma_warp && elect_sync()) {
    // Barriers are initialized once, so these phase cursors persist across work items.
    // A later kernel packs these arrays into register bitmasks.
...
#pragma unroll
  ...
  if (is_corr_warp) {
    ...
    // Role-scope arrays carry parity across work items; a later kernel packs them.
