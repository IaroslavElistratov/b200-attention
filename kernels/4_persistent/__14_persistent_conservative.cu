...
//
// This is conservative version of the persistent kernel. At the end of every
// work item, each warp stops, arrives at work_done, and the load warp waits for
// the slowest one — the whole pipeline drains. Only then invalidate and
// re-initialize every work-local barrier for the next item. The drain is not optional:
// re-initializing a barrier that some warp is still waiting on is undefined
// behavior, so rebuilding the barriers each time requires this global stop.
//
// 14_persistent.cu removes the stop entirely. We initialize the barriers once,
// let the phases keep flipping across work items, and give each buffer its own
// recycle barrier, allowing each warp role to start the next work item as soon
// as the buffers it needs are free.
// The longer each work item runs, the less its boundary matters.
//
// We keep this variant because it is the natural first implementation.
// 
// 
// 
// 
// Conceptually, one elected lane from the load warp does:

// first work item:
//     initialize the work-local barriers

// later work items:
//     wait until all 14 warp roles finish the previous work item
//     invalidate the old work-local barriers
//     initialize them again

// signal setup_done

// At the end of every work item, one lane from each warp arrives on work_done.
// Only after all 14 warps have arrived is it safe for the load warp to invalidate
// and reset the barriers.

// The MMA, softmax, and correction warps wait on setup_done before touching the
// new work item’s barrier state. This prevents them from accessing a barrier while
// it is invalid or being reinitialized.

// The load warp owns this setup because it already starts each work item and then
// immediately produces q_ready and kv_ready. Another lane could
// have done it, as long as it preserved the same ordering.




// Kernel 14 focus: persistent CTA scheduling across output work items
//   1. Launch up to one CTA per SM, then let each CTA process multiple output work items
//   2. Fold in role-local loops and __launch_bounds__(TB_SIZE, 1)
//   3. Keep a conservative boundary: all roles finish before the next work item starts
...
__global__ __launch_bounds__(TB_SIZE, 1) void attention_tcgen05_v14_kernel(
  ...
  //
  // Barriers used between persistent output work items:
  //   setup-done:         load warp -> compute warps; this output work item's barrier setup is
  //                       done; its role-local loops begin afterward
  //   work-done:          all warp roles -> load; current work is done, so load invalidates
  //                       its work-local barriers before the next work item
  __shared__ uint64_t mbar_q_ready[1];
  // Kernel 14 initializes and reinitializes the work-local barriers inside the
  // output-work-item loop. These two barriers signal when setup is complete and
  // when it is safe to reset the work-local barriers for the next work item.
  __shared__ uint64_t mbar_setup_done[1];
  __shared__ uint64_t mbar_work_done[1];
  ...
  const int mbar_work_done_addr = static_cast<int>(__cvta_generic_to_shared(mbar_work_done));
  ...
  if (is_load_warp && elect_sync()) {
    // Kernel 13 initialized all work-local barriers once because each CTA processed only one
    ...
    // for the first item, then invalidated and reinitialized for every later item. Only these two
    // cross-work boundary handoffs persist across output work items and therefore are initialized
    // once here, outside the loop over output work items.
    //
    // setup_done expects one arrival after the work-local barriers are ready.
    // work_done counts completed warp roles, not threads, so its NUM_WARPS expected
    // arrivals mean one designated lane from each warp arrives once. Its phase
    // advances only after every warp role finishes the current work item.
    mbarrier::init(mbar_setup_done_addr, 1);
    mbarrier::init(mbar_work_done_addr, NUM_WARPS);
  ...
  // containing every role. Roles run independently within one output work item,
  // but all roles finish it before any role starts the next one.
  //
  ...
  if (is_load_warp && elect_sync()) {
    ...
    for (int work_id = blockIdx.x, work_iter = 0; work_id < total_work_items;
      ...
      //
      // After setup, the coordinator signals setup_done; the other roles then begin this output
      ...
      // work_done -> setup_done ordering. The load lane owns setup so it can start Q/KV loads
      // and publish q_ready/kv_ready without waiting on a setup handoff from another warp.

      // 1) Barrier teardown
      //
      // A previous output work item exists, so wait until all roles finish it before
      // invalidating and reinitializing the work-local barriers.
      if (work_iter > 0) {
        mbarrier::wait(mbar_work_done_addr, phase_work_done);
        phase_work_done ^= 1;
      ...
      // This replaces the __syncthreads() after one-time initialization in kernel 13.
      ...
      // This is only one of the work_done arrivals; it does not advance the barrier by itself.
      mbarrier::arrive(mbar_work_done_addr);
    }
  }
...
#pragma unroll
            ...
            if (score_n8 >= FIRST_CACHED_FRAGMENT) {
              // this pass visits all sixteen x8 fragments (128 scores) but caches only
              // the final twelve (96 scores), so rebase row fragments 4..15 to cache slots 0..11
              const int cache_fragment = score_n8 - FIRST_CACHED_FRAGMENT;
#pragma unroll
              for (int i = 0; i < 8; ++i) {
                cached_scores[cache_fragment][i] = s8[i];
              }
            }
          }
        }
...
#pragma unroll
      ...
      const int final_pv_phase = (kv_tiles - 1) & 1;
...
#pragma unroll
      ...
      // TODO: __syncwarp()?
      // work_done counts completed warp roles, not threads: one arrival represents
      // this whole warp finishing its work. It expects NUM_WARPS arrivals, so only
      // lane 0 arrives. If every lane arrived, one warp could advance the phase
      // before the other warp roles finished.
      if (lane_id == 0) {
        mbarrier::arrive(mbar_work_done_addr);
      }
    }
...
#pragma unroll
  ...
  // deallocates TMEM;
  // see cbdb710, b784625 for superceded deallocation attempts
  __syncthreads();
  if (is_mma_warp) {
...
void attention_tcgen05_v14_launch(const nv_bfloat16* Q_ptr, const nv_bfloat16* K_ptr,
  ...
  if (smem_bytes > 48 * 1024) {
    check_cuda(cudaFuncSetAttribute(attention_tcgen05_v14_kernel,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    int(smem_bytes)));
  }

  attention_tcgen05_v14_kernel<<<grid, TB_SIZE, smem_bytes, stream>>>(
