// Baseline FA4-style single-Q-tile kernel.
//
...
__global__ __launch_bounds__(TB_SIZE) void attention_tcgen05_v1_kernel(
...
void attention_tcgen05_v1_launch(const nv_bfloat16* Q_ptr, const nv_bfloat16* K_ptr,
  ...
  if (smem_bytes > 48 * 1024) {
    check_cuda(cudaFuncSetAttribute(attention_tcgen05_v1_kernel,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    int(smem_bytes)));
  }

  attention_tcgen05_v1_kernel<<<grid, TB_SIZE, smem_bytes, stream>>>(
