// Kernel 3 focus:
//   1. Keep kernel 2 behavior (P in TMEM, PV A from TMEM, no explicit V->Vt transpose).
//   2. Upgrade Q/K/V operand contracts to SWIZZLE_128B + wide (64-col) TMA boxes.
//   3. Keep O as TMEM accumulator/output (TMEM layout-D path unchanged).
//
//   1. Add MN-major swizzled descriptor helper:
//       - swizzle128::desc_mnmajor(...)
//   2. Switch V TMA map to swizzled wide box:
//       - shared_height=8, shared_width=64, CU_TENSOR_MAP_SWIZZLE_128B.
//   3. Reworked V load loop to swizzled 8x64 atom packing:
//       - k8 over BLOCK_N/8
//       - n64 over HEAD_DIM/64
//       - dst_v = V_smem + k8 * k8_stride + n64 * n64_stride.
//   4. Switched PV B descriptor to use swizzled MN-major helper:
//       - build the base swizzle128::desc_mnmajor(V_smem, HEAD_DIM)
//         once, then step the descriptor cursor per PV microstep.
...
__global__ __launch_bounds__(TB_SIZE) void attention_tcgen05_v3_kernel(
  ...
  // Load Q once into SMEM as 2 wide (64 bf16 = 128B) swizzled chunks.
  if (warp_id == 0 && elect_sync()) {
    swizzle128::tma_load_kmajor(Q_smem, &Q_tmap, BLOCK_M, q_row0, mbar_addr);
  ...
  for (int kv_tile = 0; kv_tile < kv_tiles; ++kv_tile) {
    ...
    if (warp_id == 0 && elect_sync()) {
      swizzle128::tma_load_kmajor(K_smem, &K_tmap, BLOCK_N, kv_row0, mbar_addr);
      swizzle128::tma_load_mnmajor(V_smem, &V_tmap, kv_row0, mbar_addr);
    ...
    // QK^T -> S (TMEM), with K-major SW128 descriptor math.
    if (warp_id == 0 && elect_sync()) {
...
#pragma unroll
    ...
    // Same packed-P TMEM pass as kernel 2.
...
void attention_tcgen05_v3_launch(const nv_bfloat16* Q_ptr, const nv_bfloat16* K_ptr,
  ...
  if (smem_bytes > 48 * 1024) {
    check_cuda(cudaFuncSetAttribute(attention_tcgen05_v3_kernel,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    int(smem_bytes)));
  }

  attention_tcgen05_v3_kernel<<<grid, TB_SIZE, smem_bytes, stream>>>(
